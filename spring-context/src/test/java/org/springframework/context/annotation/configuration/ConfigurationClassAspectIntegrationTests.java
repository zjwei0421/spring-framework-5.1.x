 ...
 * annotation-com.qinghe.config or component-scan), and thus will also use {@code <aop:aspectj-autoproxy/>} to enable
...
public class ConfigurationClassAspectIntegrationTests {
	...
	private void assertAdviceWasApplied(Class<?> configClass) {
		...
		new XmlBeanDefinitionReader(factory).loadBeanDefinitions(
				new ClassPathResource("aspectj-autoproxy-com.qinghe.config.xml", ConfigurationClassAspectIntegrationTests.class));
		GenericApplicationContext ctx = new GenericApplicationContext(factory);
		ctx.addBeanFactoryPostProcessor(new ConfigurationClassPostProcessor());
		ctx.registerBeanDefinition("com.qinghe.config", new RootBeanDefinition(configClass));
		ctx.refresh();

...
public class ImportTests {
	...
	public void testProcessImportsWithAsm() {
		...
		DefaultListableBeanFactory beanFactory = new DefaultListableBeanFactory();
		beanFactory.registerBeanDefinition("com.qinghe.config", new RootBeanDefinition(ConfigurationWithImportAnnotation.class.getName()));
		ConfigurationClassPostProcessor pp = new ConfigurationClassPostProcessor();
		pp.postProcessBeanFactory(beanFactory);
	...
	public void testImportAnnotationWithMultipleArgumentsResultingInOverriddenBeanDefinition() {
		DefaultListableBeanFactory beanFactory = new DefaultListableBeanFactory();
		beanFactory.registerBeanDefinition("com.qinghe.config", new RootBeanDefinition(
	...
	public void reproSpr9023() {
		...
		assertThat(ctx.getBeanNamesForType(B.class)[0], is("com.qinghe.config-b"));
		assertThat(ctx.getBeanNamesForType(A.class)[0], is("com.qinghe.config-a"));
	}

	@Configuration("com.qinghe.config-a")
	static class A { }

	@Configuration("com.qinghe.config-b")
	@Import(A.class)
	static class B { }

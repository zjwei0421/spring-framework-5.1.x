...
public class JdbcNamespaceIntegrationTests {
	...
	public void createEmbeddedDatabase() throws Exception {
		...
		assertCorrectSetup("jdbc-com.qinghe.config.xml", "dataSource", "h2DataSource", "derbyDataSource");
	}
	...
	public void createEmbeddedDatabaseAgain() throws Exception {
		...
		Assume.group(TestGroup.LONG_RUNNING);
		assertCorrectSetup("jdbc-com.qinghe.config.xml", "derbyDataSource");
	}

	@Test
	public void createWithResourcePattern() throws Exception {
		assertCorrectSetup("jdbc-com.qinghe.config-pattern.xml", "dataSource");
	}

	@Test
	public void createWithAnonymousDataSourceAndDefaultDatabaseName() throws Exception {
		assertCorrectSetupForSingleDataSource("jdbc-com.qinghe.config-db-name-default-and-anonymous-datasource.xml",
			(url) -> url.endsWith(DEFAULT_DATABASE_NAME));
	}

	@Test
	public void createWithImplicitDatabaseName() throws Exception {
		assertCorrectSetupForSingleDataSource("jdbc-com.qinghe.config-db-name-implicit.xml", (url) -> url.endsWith("dataSource"));
	}

	@Test
	public void createWithExplicitDatabaseName() throws Exception {
		assertCorrectSetupForSingleDataSource("jdbc-com.qinghe.config-db-name-explicit.xml", (url) -> url.endsWith("customDbName"));
	}
	...
	public void createWithGeneratedDatabaseName() throws Exception {
		...
		assertCorrectSetupForSingleDataSource("jdbc-com.qinghe.config-db-name-generated.xml", urlPredicate);
	}

	@Test
	public void createWithEndings() throws Exception {
		assertCorrectSetupAndCloseContext("jdbc-initialize-endings-com.qinghe.config.xml", 2, "dataSource");
	}

	@Test
	public void createWithEndingsNested() throws Exception {
		assertCorrectSetupAndCloseContext("jdbc-initialize-endings-nested-com.qinghe.config.xml", 2, "dataSource");
	}

	@Test
	public void createAndDestroy() throws Exception {
		ClassPathXmlApplicationContext context = context("jdbc-destroy-com.qinghe.config.xml");
		try {
	...
	public void createAndDestroyNestedWithHsql() throws Exception {
		ClassPathXmlApplicationContext context = context("jdbc-destroy-nested-com.qinghe.config.xml");
		try {
			DataSource dataSource = context.getBean(DataSource.class);
			JdbcTemplate template = new JdbcTemplate(dataSource);
			assertNumRowsInTestTable(template, 1);
	...
	public void createAndDestroyNestedWithH2() throws Exception {
		ClassPathXmlApplicationContext context = context("jdbc-destroy-nested-com.qinghe.config-h2.xml");
		try {
			DataSource dataSource = context.getBean(DataSource.class);
			JdbcTemplate template = new JdbcTemplate(dataSource);
			assertNumRowsInTestTable(template, 1);
	...
	public void multipleDataSourcesHaveDifferentDatabaseNames() throws Exception {
		...
		new XmlBeanDefinitionReader(factory).loadBeanDefinitions(new ClassPathResource(
			"jdbc-com.qinghe.config-multiple-datasources.xml", getClass()));
	...
	public void embeddedWithCustomSeparator() throws Exception {
		assertCorrectSetupAndCloseContext("jdbc-com.qinghe.config-custom-separator.xml", 2, "dataSource");
	}

...
* <<beans-annotation-com.qinghe.config,Annotation-based configuration>>: Spring 2.5 introduced
...
example, `file:C:/com.qinghe.config/services.xml` or `classpath:/com.qinghe.config/services.xml`. However, be
...
----
	<bean id="mappings"
		class="org.springframework.beans.factory.com.qinghe.config.PropertyPlaceholderConfigurer">
...
* Implement the more fine-grained control available with annotation-based configuration,
  as described in <<beans-annotation-com.qinghe.config>>.
...
`org.springframework.beans.factory.com.qinghe.config` package) to be useful.
====
...
`org.springframework.beans.factory.com.qinghe.config.Scope` interface, which is described in this
section. For an idea of how to implement your own scopes, see the `Scope`
implementations that are supplied with the Spring Framework itself and the
{api-spring-framework}/beans/factory/com.qinghe.config/Scope.html[`Scope`] javadoc,
...
See the {api-spring-framework}/beans/factory/com.qinghe.config/Scope.html#registerDestructionCallback[javadoc]
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<bean class="org.springframework.beans.factory.com.qinghe.config.CustomScopeConfigurer">
...
{api-spring-framework}/beans/factory/com.qinghe.config/BeanPostProcessor.html[`BeanPostProcessor`]
...
The `org.springframework.beans.factory.com.qinghe.config.BeanPostProcessor` interface consists of
...
class itself or at least the `org.springframework.beans.factory.com.qinghe.config.BeanPostProcessor`
...
----
	...
	import org.springframework.beans.factory.com.qinghe.config.BeanPostProcessor;
...
`org.springframework.beans.factory.com.qinghe.config.BeanFactoryPostProcessor`. The semantics of
...
{api-spring-framework}/beans/factory/com.qinghe.config/BeanFactoryPostProcessor.html[`BeanFactoryPostProcessor`] and {api-spring-framework}/core/Ordered.html[`Ordered`] interfaces for more details.
...
[subs="verbatim,quotes"]
----
	<bean class="org.springframework.beans.factory.com.qinghe.config.PropertyPlaceholderConfigurer">
...
See the {api-spring-framework}/beans/factory/com.qinghe.config/PropertyPlaceholderConfigurer.html[`PropertyPlaceholderConfigurer`] javadoc for more information.
...
[[beans-annotation-com.qinghe.config]]
...
[NOTE]
====
`<context:annotation-com.qinghe.config/>` only looks for annotations on beans in the same
application context in which it is defined. This means that, if you put
`<context:annotation-com.qinghe.config/>` in a `WebApplicationContext` for a `DispatcherServlet`,
...
(<<beans-annotation-com.qinghe.config>>) demonstrates how to provide a lot of the configuration
...
`<context:annotation-com.qinghe.config>`. There is usually no need to include the
`<context:annotation-com.qinghe.config>` element when using `<context:component-scan>`.
...
`CommonAnnotationBeanPostProcessor` by including the annotation-com.qinghe.config attribute
...
----
	<web-app>
		...
		<servlet>
			...
			<!-- Again, com.qinghe.config locations must consist of one or more comma- or space-delimited
...
[[beans-java-further-information-java-com.qinghe.config]]
...
----
	...
	public class ServiceConfig {
		...
		@Bean
		public TransferService transferService() {
			// navigate 'through' the com.qinghe.config class to the @Bean method!
...
----
	...
	@Configuration
	@Import({ServiceConfig.class, DefaultRepositoryConfig.class})  // import the concrete com.qinghe.config!
	public class SystemTestConfig {
...
include it within `system-test-com.qinghe.config.xml` as a `<bean/>` definition. Because
`<context:annotation-com.qinghe.config/>` is switched on, the container recognizes the
...
The following example shows part of a sample `system-test-com.qinghe.config.xml` file:
...
----
	public static void main(String[] args) {
		ApplicationContext ctx = new ClassPathXmlApplicationContext("classpath:/com/acme/system-test-com.qinghe.config.xml");
		TransferService transferService = ctx.getBean(TransferService.class);
		// ...
	}
----
...
NOTE: In `system-test-com.qinghe.config.xml` file, the `AppConfig` `<bean/>` does not declare an `id`
...
describe in the previous example, we can redefine `system-test-com.qinghe.config.xml` to take advantage of component-scanning.
Note that, in this case, we need not explicitly declare
`<context:annotation-com.qinghe.config/>`, because `<context:component-scan/>` enables the same
functionality.

The following example shows the modified `system-test-com.qinghe.config.xml` file:
...
----
	@Configuration
	@ImportResource("classpath:/com/acme/properties-com.qinghe.config.xml")
	public class AppConfig {
...
[subs="verbatim,quotes"]
----
	properties-com.qinghe.config.xml
	<beans>
		<context:property-placeholder location="classpath:/com/acme/jdbc.properties"/>
	</beans>
----
...
----
	<beans profile="development"
		...
		<jdbc:embedded-database id="dataSource">
			<jdbc:script location="classpath:com/bank/com.qinghe.config/sql/schema.sql"/>
			<jdbc:script location="classpath:com/bank/com.qinghe.config/sql/test-data.sql"/>
...
is populated with additional default property sources including servlet com.qinghe.config and servlet
...
----
	<beans>
		<import resource="com/bank/service/${customer}-com.qinghe.config.xml"/>
	</beans>
----
====

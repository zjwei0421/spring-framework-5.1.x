...
<<webflux-com.qinghe.config, WebFlux infrastructure>> and <<webflux-httphandler, run it>> with a few
lines of code.
...
`WebHandler`. When using the <<webflux-com.qinghe.config>>, registering a `WebFilter` is as simple
...
<<webflux-com.qinghe.config>>, registering a `WebExceptionHandler` is as simple as declaring it as a
...
| Extension of `ResponseStatusExceptionHandler` that can also determine the HTTP status
  ...
  This handler is declared in the <<webflux-com.qinghe.config>>.
...
<<webflux-com.qinghe.config-message-codecs>>.
...
[[webflux-framework-com.qinghe.config]]
=== WebFlux Config
[.small]#<<web.adoc#mvc-servlet-com.qinghe.config, Same as in Spring MVC>>#
...
However, in most cases, the <<webflux-com.qinghe.config>> is the best starting point. It declares the
required beans and provides a higher-level configuration callback API to customize it.

NOTE: Spring Boot relies on the WebFlux com.qinghe.config to configure Spring WebFlux and also provides
...
`HandlerResultHandler` implementations, all of which are declared in the <<webflux-com.qinghe.config>>:
...
to your Spring configuration. <<webflux-com.qinghe.config-view-resolvers, WebFlux Config>> provides a
...
views through the <<webflux-com.qinghe.config-view-resolvers, WebFlux Configuration>>. Default views are
...
You can use the <<webflux-com.qinghe.config-message-codecs>> option of the <<webflux-com.qinghe.config>> to
...
<<webflux-com.qinghe.config-conversion, WebFlux Java configuration>> to register `Converter` and
...
for optimal performance. See the section on configuring <<webflux-com.qinghe.config-static-resources>>.




[[webflux-com.qinghe.config]]
== WebFlux Config
[.small]#<<web.adoc#mvc-com.qinghe.config, Same as in Spring MVC>>#
...
<<webflux-com.qinghe.config-advanced-java>>.



[[webflux-com.qinghe.config-enable]]
=== Enabling WebFlux Config
[.small]#<<web.adoc#mvc-com.qinghe.config-enable, Same as in Spring MVC>>#

You can use the `@EnableWebFlux` annotation in your Java com.qinghe.config, as the following example shows:
...
[[webflux-com.qinghe.config-customize]]
=== WebFlux com.qinghe.config API
[.small]#<<web.adoc#mvc-com.qinghe.config-customize, Same as in Spring MVC>>#
...
[[webflux-com.qinghe.config-conversion]]
=== Conversion, formatting
[.small]#<<web.adoc#mvc-com.qinghe.config-conversion, Same as in Spring MVC>>#
...
[[webflux-com.qinghe.config-validation]]
=== Validation
[.small]#<<web.adoc#mvc-com.qinghe.config-validation, Same as in Spring MVC>>#
...
mark it with `@Primary` in order to avoid conflict with the one declared in the MVC com.qinghe.config.



[[webflux-com.qinghe.config-content-negotiation]]
=== Content Type Resolvers
[.small]#<<web.adoc#mvc-com.qinghe.config-content-negotiation, Same as in Spring MVC>>#
...
[[webflux-com.qinghe.config-message-codecs]]
=== HTTP message codecs
[.small]#<<web.adoc#mvc-com.qinghe.config-message-converters, Same as in Spring MVC>>#
...
[[webflux-com.qinghe.config-view-resolvers]]
=== View Resolvers
[.small]#<<web.adoc#mvc-com.qinghe.config-view-resolvers, Same as in Spring MVC>>#
...
[[webflux-com.qinghe.config-static-resources]]
=== Static Resources
[.small]#<<web.adoc#mvc-com.qinghe.config-static-resources, Same as in Spring MVC>>#
...
[[webflux-com.qinghe.config-path-matching]]
=== Path Matching
[.small]#<<web.adoc#mvc-com.qinghe.config-path-matching, Same as in Spring MVC>>#

You can customize options related to path matching. For details on the individual options, see the
{api-spring-framework}/web/reactive/com.qinghe.config/PathMatchConfigurer.html[`PathMatchConfigurer`] javadoc.
...
[.small]#<<web.adoc#mvc-com.qinghe.config-advanced-java, Same as in Spring MVC>>#
